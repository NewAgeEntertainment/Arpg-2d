    ...
    public class QuestNode
    {
        ...
        public void SetState(QuestNodeState newState, bool informListeners = true)
        {
            ...
            if (!informListeners)
            {
                ...
                SetConditionChecking(m_state == QuestNodeState.Active);
                return;
            }
            ...
            // Note: State actions might change this node's state, so check m_state, not newState:
            SetConditionChecking(m_state == QuestNodeState.Active);
        ...
        {
            if (quest == null || parent == null) return;
            var questState = quest.GetState();
            var parentState = parent.GetState();
            var myState = GetState();
            if (questState == QuestState.Active &&
                parentState == QuestNodeState.True &&
                myState == QuestNodeState.Inactive)
            {
                SetState(QuestNodeState.Active);
            }
            // Special exception: If parent became true and quest is already successful,
            // and if this node is a Success node, we can set it true.
            else if (questState == QuestState.Successful &&
                parentState == QuestNodeState.True &&
                nodeType == QuestNodeType.Success)
            {
                SetState(QuestNodeState.True);
            }
            // Special exception: Or if parent became true and quest is already failed,
            // and if this node is a Failure node, we can set it true.
            else if (questState == QuestState.Failed &&
                parentState == QuestNodeState.True &&
                nodeType == QuestNodeType.Failure)
            {
                SetState(QuestNodeState.True);
            }
        }
