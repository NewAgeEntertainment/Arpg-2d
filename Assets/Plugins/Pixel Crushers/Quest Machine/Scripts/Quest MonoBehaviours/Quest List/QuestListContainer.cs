...
namespace PixelCrushers.QuestMachine
{
    ...
    public class QuestListContainer : Saver, IMessageHandler
    {
        ...
        protected void InstantiateQuestAssets()
        {
            ...
            ResetOriginalQuestListNumTrueConditionsValues();
            AddQuests(originalQuestList, true);
        }

        protected void ResetOriginalQuestListNumTrueConditionsValues()
        {
            foreach (var quest in originalQuestList)
            {
                if (quest == null) continue;
                quest.offerConditionSet.numTrueConditions = 0;
                foreach (var questNode in quest.nodeList)
                {
                    if (questNode == null) continue;
                    questNode.conditionSet.numTrueConditions = 0;
                }
            }
        }
